...
* `AsianPct`, `BlackPct`, `HispanicPct`, `IndigenousPct`, `WhitePct`: Race and demographic control variables (% Asian, % Black, % Hispanic, % Indigenous, % White)
...
The 4 coefficients that are statistically significant are: Dose1Pct, CarPct, IncomePerCap, and HispanicPct. One interesting note is that although HispanicPct is statistically significant, none of the other ethnic groups in this analysis are statistically significant. Given this context, we are hesitant to trust HispanicPct as practically significant. Further we note that IncomePerCap is less than 0.00001 and of limited practical significance. Overall, all coefficients were small, indicating that the relationship between them and travel is weak. 

## CLM Assumptions

While the size of our dataset (over 2,600 observations) is large enough that we need not use the Classical Linear model approach, we have completed an analysis of the CLM assumptions on model #2 as a validation exercise. The most interesting/relevant assumptions are highlighted below. We saw no issues with collinearity during the model building process. With regards to Independent & Identically Distributed (IID) data, we realize that real world data can never be perfect. However, with a large sample and diverse covariates, we feel our data are sufficiently IID.
...
It is difficult to assess whether our model has a linear conditional expectation based on the plot of predicted vs residual values for model 2. The trend line certainly appears quite linear, however the tight distribution of data points suggests the need for a further transformation of one of our variables to create more opportunity for our model to create linear predictions. 
...
The histogram of residuals and the qqplot shows some deviation from normality, specifically a right skew and perhaps an unusual concentration on the right tail.
...
# Omitted Variable Bias and Variable Limitations
